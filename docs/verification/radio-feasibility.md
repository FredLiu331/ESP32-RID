...
状态：`PASS_DRIVER_GATES`。2026-07-21 在 ESP32-C5 rev1.2 实机上，当时定义的七项驱动/API
...
首次规格审查还发现 Beacon 将零长度 SSID IE 错写成零长度 Country IE，提交
`99c1438` 完成修正。早期探针曾包含 NAN Action 试验，但产品需求随后明确放弃 NAN
承载和 NAN 原始帧仿真；当前探针、公共 API 和验收入口均不再构造或发射 NAN 帧。
...
实机判定还必须用受控试验环境中的抓包或目标 RID 接收机确认 Beacon 帧实际位于
