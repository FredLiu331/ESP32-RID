...
| 天线 | 按用户要求跳过记录；型号、端口、增益和摆放均未采集，本次结果不具备天线条件复现性 |
...
| 实测固件 | `99c1438`；`radio_probe.bin` 大小 `0x12afc0` |
...
首次规格审查还发现 Beacon 将零长度 SSID IE 错写成零长度 Country IE，NAN Public
Action 也只有固定头、没有 NAN Attribute。提交 `99c1438` 将 Beacon 修正为零长度
SSID IE，并为 NAN Action 加入 Master Indication Attribute（ID 0、两字节小端长度 2、
Master Preference 100、Random Factor 1）；编译期校验会拒绝截断 IE、缺失 Attribute
和错误长度。修正后的固件重新烧录后，七项关卡仍全部通过。

ESP-IDF 自带 Python 环境未安装 `pytest` 与 `pytest-embedded`。测试依赖安装在独立
`/tmp/esp32-rid-pytest-env` 虚拟环境，避免改变 ESP-IDF v5.5.4 使用的 esptool 4.12.0。
项目验收入口连接同一实机并跳过重复烧录，合法帧修正后的结果为
`1 passed in 5.83s`。测试出现两个非失败警告：未注册 `esp32c5` marker，以及插件
experimental API 提示。
...
构建结果：成功。实测固件 `radio_probe.bin` 大小 `0x12afc0`，factory app 分区
`0x177000`，剩余 `0x4c040`（20%）。

实机自动化命令：

```bash
env IDF_PATH=/home/tomato/esp/esp-idf-v5.5.4 \
  /tmp/esp32-rid-pytest-env/bin/python -m pytest -q \
  pytest_rid/test_radio_probe.py --embedded-services esp,idf --target esp32c5 \
  --app-path test_apps/radio_probe \
  --port /dev/serial/by-id/usb-1a86_USB_Single_Serial_5B90156485-if00 \
  --skip-autoflash y -o cache_dir=/tmp/esp32-rid-pytest-cache
```
...
将项目测试依赖安装在独立虚拟环境，再连接开发板并明确指定串口。不要把 pytest 插件
安装到 ESP-IDF v5.5.4 自带环境，以免依赖解析改变其 esptool 4.12.0：
...
python3 -m venv /tmp/esp32-rid-pytest-env
/tmp/esp32-rid-pytest-env/bin/python -m pip install \
  pytest==9.1.1 pytest-embedded-idf==2.8.1 pytest-embedded-serial-esp==2.8.1
env IDF_PATH=/home/tomato/esp/esp-idf-v5.5.4 \
  /tmp/esp32-rid-pytest-env/bin/python -m pytest -q \
  pytest_rid/test_radio_probe.py --embedded-services esp,idf --target esp32c5 \
  --app-path test_apps/radio_probe \
