...
RuntimeError Runtime::apply(const SystemConfig &config, uint64_t now_ms) {
    ...
    for (size_t i = 0; i < aircraft_.size(); ++i) next_due_[i].fill(now_ms);
    stats_ = {};
    started_ms_ = now_ms;
    sequence_ = 0;
    counter_ = 0;
...
RuntimeError Runtime::tick(uint64_t now_ms) {
    ...
    for (size_t i = 0; i < aircraft_.size(); ++i) {
        ...
        if (aircraft.protocol == Protocol::OpenDroneId && wifi) {
            bool due = false;
            ...
            if (!encoded.ok() ||
                build_opendroneid_beacon(frame, address.data(), ssid, counter_++,
